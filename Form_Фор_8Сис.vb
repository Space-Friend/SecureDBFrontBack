...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_Фор_8Сис"
...
Private Sub Кнопка102_Click()
        ...
        Dim rs As Recordset
        Set db = CurrentDb
        Set rs = db.OpenRecordset("Зап_8сис")
...
Public Sub Кнопка65_Click()
    ...
    Forms![Фор_Статус].Namesys = "Система термостатирования АКС 1.04.0000. П2"
    Forms![Фор_Статус].Num = DLookup("[Ном]", "Зап_8сис", "[Ном] = " & Forms!Фор_8Сис!Ном)
    Forms![Фор_Статус].Tip = ""
    On Error GoTo ohsht
    Forms![Фор_Статус].Oboz = DLookup("[Обознач]", "Зап_8сис", "[Ном] = " & Forms!Фор_8Сис!Ном)
    Forms![Фор_Статус].EveryoneCanSeeMe = Forms![Фор_Статус].lll.Caption
    DoCmd.OpenForm "Фор_Склад_Окно", , , , acFormReadOnly, acDialog, "Зап_8сис"
    DoCmd.RefreshRecord
...
ohsht:
    If MsgBox("У агрегата отсутствует обозначение, чтобы выполнить поиск аналогов. Выполнить поиск по типу?", vbYesNo) = vbYes Then
        Forms![Фор_Статус].Tip = DLookup("[ТипАг]", "Зап_8сис", "[Ном] = " & Forms!Фор_8Сис!Ном)
        Forms![Фор_Статус].EveryoneCanSeeMe = Forms![Фор_Статус].lll.Caption
        DoCmd.OpenForm "Фор_Склад_Окно", , , , acFormReadOnly, acDialog, "Зап_8сис"
        DoCmd.RefreshRecord
    End If
