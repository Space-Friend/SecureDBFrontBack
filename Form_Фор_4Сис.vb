...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_Фор_4Сис"
...
Private Sub Кнопка102_Click()
        ...
        Dim rs As Recordset
        Set db = CurrentDb
        Set rs = db.OpenRecordset("Зап_4сис")
...
Public Sub Кнопка65_Click()
    ...
    Forms![Фор_Статус].Namesys = "Пульт пневмо гидро испытаний бокс 2 1.04.0000.141 С2"
    Forms![Фор_Статус].Num = DLookup("[Ном]", "Зап_4сис", "[Ном] = " & Forms!Фор_4Сис!Ном)
    Forms![Фор_Статус].Tip = ""
    On Error GoTo ohsht
    Forms![Фор_Статус].Oboz = DLookup("[Обознач]", "Зап_4сис", "[Ном] = " & Forms!Фор_4Сис!Ном)
    Forms![Фор_Статус].EveryoneCanSeeMe = Forms![Фор_Статус].lll.Caption
    DoCmd.OpenForm "Фор_Склад_Окно", , , , acFormReadOnly, acDialog, "Зап_4сис"
    DoCmd.RefreshRecord
...
ohsht:
    If MsgBox("У агрегата отсутствует обозначение, чтобы выполнить поиск аналогов. Выполнить поиск по типу?", vbYesNo) = vbYes Then
        Forms![Фор_Статус].Tip = DLookup("[ТипАг]", "Зап_4сис", "[Ном] = " & Forms!Фор_4Сис!Ном)
        Forms![Фор_Статус].EveryoneCanSeeMe = Forms![Фор_Статус].lll.Caption
        DoCmd.OpenForm "Фор_Склад_Окно", , , , acFormReadOnly, acDialog, "Зап_4сис"
        DoCmd.RefreshRecord
    End If
