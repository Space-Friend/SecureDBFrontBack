...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_Фор_7Сис"
...
Private Sub Кнопка102_Click()
        ...
        Dim rs As Recordset
        Set db = CurrentDb
        Set rs = db.OpenRecordset("Зап_7сис")
...
Public Sub Кнопка65_Click()
    ...
    Forms![Фор_Статус].Namesys = "Шкаф ЭПК системы окислителя 1.04.0360.082.П2"
    Forms![Фор_Статус].Num = DLookup("[Ном]", "Зап_7сис", "[Ном] = " & Forms!Фор_7Сис!Ном)
    Forms![Фор_Статус].Tip = ""
    On Error GoTo ohsht
    Forms![Фор_Статус].Oboz = DLookup("[Обознач]", "Зап_7сис", "[Ном] = " & Forms!Фор_7Сис!Ном)
    Forms![Фор_Статус].EveryoneCanSeeMe = Forms![Фор_Статус].lll.Caption
    DoCmd.OpenForm "Фор_Склад_Окно", , , , acFormReadOnly, acDialog, "Зап_7сис"
    DoCmd.RefreshRecord
...
ohsht:
    If MsgBox("У агрегата отсутствует обозначение, чтобы выполнить поиск аналогов. Выполнить поиск по типу?", vbYesNo) = vbYes Then
        Forms![Фор_Статус].Tip = DLookup("[ТипАг]", "Зап_7сис", "[Ном] = " & Forms!Фор_7Сис!Ном)
        Forms![Фор_Статус].EveryoneCanSeeMe = Forms![Фор_Статус].lll.Caption
        DoCmd.OpenForm "Фор_Склад_Окно", , , , acFormReadOnly, acDialog, "Зап_7сис"
        DoCmd.RefreshRecord
    End If
